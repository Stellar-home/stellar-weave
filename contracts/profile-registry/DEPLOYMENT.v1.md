> ⚠️ **Deprecated — v1 deployment (historical record only)**
> This contract (`CAVUZWNQ322DFBNDEENP6GBYF6ESZFQDIEJN5C367WIG23AFMZO7ZLDU`) has been
> superseded by ProfileRegistry v2. It remains live on testnet as an immutable historical
> record; the transaction hashes below are real proof-of-work and should not be erased.
> See [`DEPLOYMENT.md`](./DEPLOYMENT.md) for the current v2 deployment.
>
> Why v2 was needed: `Profile.follower_count`/`following_count` were permanently-zero
> vestigial fields (no mutation entrypoint existed), and `profile_updated` events did not
> carry the new value, forcing indexers to do an extra RPC round-trip.
